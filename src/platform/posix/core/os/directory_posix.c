...
static DIR* directory_open(i_allocator_t* allocator, string_t path)
{
  string_t normalized = path_normalize(path, allocator);
  const char* cstr = string_to_cstr(allocator, normalized);

  return opendir(cstr);
}

directory_entry_t* directory_read(string_t path, i_allocator_t* allocator)
{
  DIR* dir = directory_open(allocator, path);
  if(!dir) {
...
static walker_level_t* new_walker_level(string_t path, i_allocator_t* allocator)
{
  DIR* dir = directory_open(allocator, path);
  if(!dir) {
    log_error("%s", strerror(errno));
...
directory_walker_t directory_walk_begin(string_t path, i_allocator_t* allocator)
{
...
bool directory_walk_next(directory_walker_t* walker)
{
  while(walker->depth >= 0) {
    ...
    string_t name = string(entry->d_name);
...
bool directory_delete_recursive(string_t path)
{
  ...
  // Collect all entries (files and dirs) in post-order
  // by storing them and processing in reverse (deepest first)
  string_t* files = array_list(string_t, temp);
  string_t* dirs = array_list(string_t, temp);

  directory_walk(path, entry, temp, {
    printf(STR_FMT " %d \n", STR_ARG(entry.path), entry.type);
    if(entry.type == FILE_TYPE_DIRECTORY) {
      array_list_push(dirs, entry.path);
    }
    else {
      array_list_push(files, entry.path);
    }
  });

  // Delete all files first
  bool success = true;
  array_list_for_each(files, string_t, file)
  {
    if(!file_delete(file)) {
      success = false;
    }
  ...
  for(int i = (int)array_list_length(dirs) - 1; i >= 0; i--) {
    if(!directory_delete(dirs[i])) {
      success = false;
    }
  }
...
bool directory_exists(string_t path)
{
  ...
  DIR* dir = directory_open(temp_alloc, path);
...
string_t directory_cwd(i_allocator_t* allocator)
{
