...
void target_log_config(target_t* t)
{
  ...
  log_info_tag(tag, "  Bin directory: " STR_FMT, STR_ARG(t->bin_dir));
  log_info_tag(tag, "  Artifacts directory: " STR_FMT, STR_ARG(t->artifacts_dir));
...
static void get_sources_from_source_dirs(target_t* target, string_t dir)
{
...
target_t* target_create(allocator_t* alloc, string_t name, target_kind_t kind)
{
  ...
  target->bin_dir = string("bin");
  target->artifacts_dir = path_join(string("artifacts"), name, alloc);
...
void target_add_source_dir(target_t* t, string_t path)
{
  ...
  get_sources_from_source_dirs(t, path);
  target_add_include_ex(t, path, INCLUDE_PATH);
}

void _target_add_source_dirs(target_t* t, string_t* paths, size_t count)
{
  for(size_t i = 0; i < count; i++) {
    target_add_source_dir(t, paths[i]);
  }
}
